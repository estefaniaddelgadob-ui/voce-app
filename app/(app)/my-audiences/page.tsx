...
// Hex values — avoids Tailwind purging dynamic class strings
const PLATFORM_STYLE: Record<string, { bg: string; color: string }> = {
  instagram:  { bg: "#FDF2F8", color: "#DB2777" },
  linkedin:   { bg: "#EFF6FF", color: "#2563EB" },
  tiktok:     { bg: "#F8FAFC", color: "#334155" },
  youtube:    { bg: "#FEF2F2", color: "#DC2626" },
  twitter:    { bg: "#F0F9FF", color: "#0284C7" },
  newsletter: { bg: "#FFFBEB", color: "#D97706" },
};
const GOAL_STYLE: Record<string, { bg: string; color: string }> = {
  inspire:   { bg: "#F3F0FE", color: "#7F77DD" },
  educate:   { bg: "#ECFDF5", color: "#1D9E75" },
  entertain: { bg: "#FFF7ED", color: "#EA580C" },
  sell:      { bg: "#F0FDF4", color: "#16A34A" },
};
const FALLBACK_STYLE = { bg: "#F4F4F2", color: "#64748B" };
...
const EMPTY_FORM: AudienceForm = {
  name: "", niche: "", who_they_are: "", biggest_struggles: "",
  goals: "", content_pillars: [], primary_platform: "instagram", content_goal: "inspire",
};
...
function AudienceFormPanel({
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-[#7F77DD]/20 bg-[#7F77DD]/5 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Audience Name">
          <input required value={form.name} onChange={(e) => set("name", e.target.value)}
            placeholder='e.g. "Beginner entrepreneurs"' className={inputCls} />
        </Field>
        <Field label="Niche">
          <input value={form.niche} onChange={(e) => set("niche", e.target.value)}
            placeholder='e.g. "female entrepreneurship"' className={inputCls} />
        </Field>
        <Field label="Who they are" hint="Age range, situation, interests">
          <textarea value={form.who_they_are} onChange={(e) => set("who_they_are", e.target.value)}
            rows={2} placeholder="e.g. Women 30–45 leaving corporate to start their own business"
            className={inputCls} />
        </Field>
        <Field label="Biggest struggles" hint="What keeps them up at night">
          <textarea value={form.biggest_struggles} onChange={(e) => set("biggest_struggles", e.target.value)}
            rows={2} placeholder="e.g. Fear of failure, not knowing where to start"
            className={inputCls} />
        </Field>
        <Field label="Their goals" hint="What they want to achieve">
          <textarea value={form.goals} onChange={(e) => set("goals", e.target.value)}
            rows={2} placeholder="e.g. Financial freedom, flexible schedule"
            className={inputCls} />
        </Field>
        <Field label="Content Pillars" hint="3–5 themes, comma-separated">
          <input value={pillarsInput} onChange={(e) => setPillarsInput(e.target.value)}
            placeholder="e.g. mindset, marketing, money, systems" className={inputCls} />
        </Field>
        ...
        <Field label="Content Goal">
          <select value={form.content_goal} onChange={(e) => set("content_goal", e.target.value)}
            className={inputCls}>
            {GOALS.map((g) => (
      ...
      <div className="mt-5 flex gap-3">
        <button type="submit" disabled={saving}
          className="flex items-center gap-2 rounded-lg bg-[#7F77DD] px-4 py-2 text-sm font-medium text-white hover:bg-[#6E66CC] disabled:opacity-60 transition">
          {saving && <Loader2 className="h-4 w-4 animate-spin" />}
          Save Audience
        </button>
        <button type="button" onClick={onCancel}
          className="rounded-lg border border-[#E2E2E0] px-4 py-2 text-sm font-medium text-[#64748B] hover:bg-[#F4F4F2] transition">
          Cancel
        </button>
      </div>
    </form>
...
function AudienceCard({ audience, onEdit, onDelete }: {
  audience: Audience; onEdit: () => void; onDelete: () => void;
}) {
  const [expanded, setExpanded] = useState(false);
  const ps = PLATFORM_STYLE[audience.primary_platform] ?? FALLBACK_STYLE;
  const gs = GOAL_STYLE[audience.content_goal] ?? FALLBACK_STYLE;
    ...
    <div className="rounded-xl border border-[#E2E2E0] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {audience.niche && <p className="mt-0.5 text-xs text-[#94A3B8]">{audience.niche}</p>}
        </div>
      ...
      {audience.content_pillars?.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {audience.content_pillars.map((p) => (
            <span key={p} className="rounded-full bg-[#F4F4F2] px-2.5 py-0.5 text-xs text-[#64748B]">{p}</span>
          ))}
        </div>
      )}
      ...
      {expanded && (
        <div className="mt-3 space-y-2 border-t border-[#E2E2E0] pt-3 text-sm">
          {audience.who_they_are && (
            <p><span className="font-medium text-[#0F172A]">Who they are: </span>
              <span className="text-[#64748B]">{audience.who_they_are}</span></p>
          )}
          {audience.biggest_struggles && (
            <p><span className="font-medium text-[#0F172A]">Struggles: </span>
              <span className="text-[#64748B]">{audience.biggest_struggles}</span></p>
          )}
          {audience.goals && (
            <p><span className="font-medium text-[#0F172A]">Goals: </span>
              <span className="text-[#64748B]">{audience.goals}</span></p>
          )}
        </div>
      )}
    </div>
...
export default function MyAudiencesPage() {
  ...
  const [formState, setFormState] = useState<{
    open: boolean; editId: string | null; initial: AudienceForm;
  ...
  async function fetchAudiences() {
    try {
      ...
      const { data } = await supabase
        .from("audiences").select("*").eq("user_id", user.id)
  ...
  function openEdit(a: Audience) {
    setFormState({
      ...
      initial: {
        name: a.name, niche: a.niche, who_they_are: a.who_they_are,
        biggest_struggles: a.biggest_struggles, goals: a.goals,
        content_pillars: a.content_pillars ?? [],
        primary_platform: a.primary_platform, content_goal: a.content_goal,
      },
    });
    setError(null);
  }
    ...
    try {
      ...
      if (formState.editId) {
        const { error } = await supabase.from("audiences").update(form)
          .eq("id", formState.editId).eq("user_id", user.id);
  ...
  return (
    <div>
      ...
      {loading ? (
      ...
      ) : audiences.length === 0 && !formState.open ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#E2E2E0] bg-white py-16 text-center">
          ...
          <button onClick={openCreate}
            className="mt-4 flex items-center gap-2 rounded-lg bg-[#7F77DD] px-4 py-2 text-sm font-medium text-white hover:bg-[#6E66CC] transition">
            <Plus className="h-4 w-4" /> Add Audience
          </button>
        </div>
      ) : (
